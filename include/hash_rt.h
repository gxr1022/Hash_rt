...
public:
        ...
        : globalDepth(initialDepth), bucketCapacity(bucketCap)
    {
        ...
        for (int i = 0; i < (1 << initialDepth); ++i)
        {
            ...
            {
                dirAcq->bucket = new Bucket(bucketCap, i);
    ...
    {
        int hashValue = hashFunction(std::stoi(key), globalDepth);
    ...
    // {
    //     int hashValue = hashFunction(stoi(key), globalDepth);
    //     // cout<<"key:"<<key<< "hashvalue: "<<hashValue<<endl;
    //     when(directory[hashValue]) << [=](acquired_cown<Directory> dirAcq) mutable
    //     {
    //         auto &bucket = dirAcq->bucket;
    ...
    void splitBucket(Bucket* oldBucket, int localDepth, int dir_indx)
    {
        ...
        when(dir_array) << [=](acquired_cown_span<Directory> dirs_acq) mutable
        {
            ...
            // when(oldBucket) << [=](acquired_cown<Bucket> oldBucketAcq) mutable
            {
                ...
                auto newBucket = new Bucket(bucketCapacity, new_bucket_prefix);
                auto newBucket_o = new Bucket(bucketCapacity, cur_bucket_prefix);

                for (auto &pair : oldBucket->kvStore)
                {
                    int newHash = hashFunction(stoi(pair.first), globalDepth);

...
namespace verona::rt
{
  ...
  public:
    std::atomic<bool> has_scheduling{true};
    std::atomic<size_t> completed_count_last_batch{0}; 
    std::atomic<bool> producer_start{true};
    ...
    static void schedule(Work* w, bool fifo = true)
    {
      ...
      auto end_time = std::chrono::high_resolution_clock::now(); 
      auto duration = std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time).count();
      // Logging::cout() << "Time taken for schedule lifo: " << duration << " ns" << Logging::endl;
    }

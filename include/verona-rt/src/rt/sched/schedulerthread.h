...
namespace verona::rt
{
  ...
  private:
    ...
    {
      // Logging::cout() << "this thread id: " << core->affinity
      //                 << " Enqueue work " << w << Logging::endl;

      // std::cout << "this thread id: " << core->affinity << " Enqueue work "
      // << w
      //           << std::endl;
    ...
    static inline void schedule_lifo(Core* c, Work* w)
    {
      ...
      // asynchronous I/O.
      // Logging::cout() << "LIFO scheduling work " << w << " onto " <<
      // c->affinity
      //                 << Logging::endl;
      ...
      auto duration = std::chrono::duration_cast<std::chrono::nanoseconds>(
                        end_time - start_time)
                        .count();
      //  Logging::cout() << "thread id: " << c->affinity << " Enqueue time
      //  taken for schedule lifo: " << duration << " ns" << Logging::endl;
      // Logging::cout() << "LIFO scheduled work " << w << " onto " <<
      // c->affinity
      //                 << Logging::endl;
    ...
    void run_inner(void (*startup)(Args...), Args... args)
    {
      ...
      while ((work = get_work(batch)))
      {
        ...
        // std::cout << "the total count of insert: "
        //             << scheduler.total_count.fetch_add(1) << std::endl;
        
        

        // Logging::cout() << "thread id: " << core->affinity
        //                 << " work duration: " << work_duration << Logging::endl;

        // Logging::cout() << "thread id: " << core->affinity
        //                 << " completed_count_last_batch: "
        //                 << Scheduler::get().completed_count_last_batch.load()
        //                 << Logging::endl;
      }
      ...
      // std::cout
      // << "total work duration: " << total_work_time << std::endl;
      // std::cout << "total get_work_duration: " << get_duration <<
      // std::endl;
    ...
    Work* steal()
    {
      ...
      while (running)
      {
        ...
        {
          core->stats.steal();
          Logging::cout() << "current thread: " << core->affinity
                          << " stole work " << work << " from "
                          << victim->affinity << Logging::endl;
          return work;
        }
      ...
      // uint64_t tsc2 = Aal::tick();
      // std::cout << "thread id: " << core->affinity << " tsc2 - tsc: " << tsc2
      // - tsc << std::endl;
      return nullptr;
    }
